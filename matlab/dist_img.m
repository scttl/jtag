...
% CVS INFO %
%%%%%%%%%%%%
% $Id: dist_img.m,v 1.1 2004-04-22 16:56:44 klaven Exp $
%
% REVISION HISTORY:
% $Log: dist_img.m,v $
...
for i = y1+1:y2
    A(i,1) = min([A(i,1),A(i-1,1)+1]);
    A(y2-i+1,1) = min([A(y2-i+1,1),A(y2-i+2,1)+1]);
end

for j = x1+1:x2
    A(1,j) = min([A(1,j),A(1,j-1)+1]);
    A(1,x2-j+1) = min([A(1,x2-j+1),A(1,x2-j+2)+1]);
end

for i = y1+1:y2
    for j = x1+1:x2
        A(i,j) = min([A(i,j),A(i-1,j)+1,A(i,j-1)+1]);
        A(y2-i+1,x2-j+1) = min([A(y2-i+1,x2-j+1),A(y2-i+2,x2-j+1)+1,A(y2-i+1,x2-j+2)+1]);
    end
end
