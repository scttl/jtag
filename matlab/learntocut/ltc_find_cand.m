function cands = ltc_find_cand(pix, seg, reg_map);
...
for i = 1:length(x_valleys);
    ...
    if (nargin >= 3);
        cutline = reg_map(top:bot,cand.x);
        if (max(cutline) == 0);
            cand.valid_cut = true;
        else;
            cand.valid_cut = false;
        end;
        cutline = reg_map(1:size(pix,1),cand.x);
        if (max(cutline) == 0);
            cand.fullpage = true;
        else;
            cand.fullpage = false;
...
for i = 1:length(y_valleys);
    ...
    if (nargin >= 3);
        cutline = reg_map(cand.y,left:right);
        if (max(cutline) == 0);
            cand.valid_cut = true;
        else;
            cand.valid_cut = false;
        end;
        cutline = reg_map(cand.y,1:size(pix,2));
        if (max(cutline) == 0);
            cand.fullpage = true;
        else;
            cand.fullpage = false;
